...
RegisterCommand("ea_generateSupportFile", function(source, args, rawCommand)
	if DoesPlayerHavePermission(source,"easyadmin.manageserver") then
		...
		supportData.config = {
			...
			ea_maxWarnings = GetConvarInt("ea_maxWarnings", 2),
...
Citizen.CreateThread(function()
	...
	AddEventHandler('EasyAdmin:amiadmin', function()
		...
		local key = GetConvar("ea_MenuButton", 166)
		if RedM then
	...
	AddEventHandler('EasyAdmin:kickPlayer', function(playerId,reason)
		if DoesPlayerHavePermission(source,"easyadmin.kick") and not DoesPlayerHavePermission(playerId,"easyadmin.immune") then
		    ...
		    TriggerClientEvent('chat:addMessage', -1, {
				template = '<div style="padding: 0.6vw; margin: 0.6vw; background-color: rgba(255, 0, 0, 0.3); border-radius: 3px;">⚠️ Admin Kick:<br> {1}<br></div>',
					args = {"^1ANNOUNCEMENT: ", "" ..getName(source).. " kicked " ..getName(playerId).. " - Reason: " ..reason}
				})
		end
	end)
	...
	AddEventHandler('EasyAdmin:banPlayer', function(playerId,reason,expires)
		if playerId ~= nil then
			if (DoesPlayerHavePermission(source,"easyadmin.ban.temporary") or DoesPlayerHavePermission(source,"easyadmin.ban.permanent")) and CachedPlayers[playerId] and not DoesPlayerHavePermission(playerId,"easyadmin.immune") then
			    ...
			    TriggerClientEvent('chat:addMessage', -1, {
						template = '<div style="padding: 0.6vw; margin: 0.6vw; background-color: rgba(255, 0, 0, 0.3); border-radius: 3px;">❗ Admin Ban:<br> {1}<br></div>',
							args = {"^1ANNOUNCEMENT: ", "" ..getName(source).. " banned " ..getName(playerId).. " - Reason: " ..reason.. " " ..os.date('%d/%m/%Y 	%H:%M:%S', expires)}
						})
			end
		end
	end)
	...
	RegisterServerEvent("EasyAdmin:ForceSkin")
	AddEventHandler('EasyAdmin:ForceSkin', function(playerId, message)
		if DoesPlayerHavePermission(source,"easyadmin.kick")  then
			TriggerClientEvent('esx_skin:openSaveableMenu', playerId)
		end
	end)

	RegisterServerEvent("EasyAdmin:ClearChat")
	AddEventHandler('EasyAdmin:ClearChat', function()
		if DoesPlayerHavePermission(source,"easyadmin.kick") then
	...
	RegisterCommand("carwipe", function(source, args, rawCommand) 
		TriggerClientEvent('chat:addMessage', -1, {
			template = '<div style="padding: 0.6vw; margin: 0.6vw; background-color: rgba(0, 102, 255, 0.4); border-radius: 3px;">🚗Car Wipe🚗<br> {1}<br></div>',
				args = {"^1ANNOUNCEMENT: ", "Car Wipe In 20 Seconds!", table.concat(args, " ")}
				})
				
		Citizen.Wait(20000)
		TriggerClientEvent("wld:carwipe", -1) 
		TriggerClientEvent('chat:addMessage', -1, {
			template = '<div style="padding: 0.6vw; margin: 0.6vw; background-color: rgba(0, 102, 255, 0.4); border-radius: 3px;">🚗Car Wipe🚗:<br> {1}<br></div>',
	...
	RegisterServerEvent("EasyAdmin:Carwipe")
	AddEventHandler('EasyAdmin:Carwipe', function()
		if DoesPlayerHavePermission(source,"easyadmin.kick") then
			TriggerClientEvent('chat:addMessage', -1, {
				template = '<div style="padding: 0.6vw; margin: 0.6vw; background-color: rgba(0, 102, 255, 0.4); border-radius: 3px;">🚗Car Wipe🚗:<br> {1}<br></div>',
					args = {"^1ANNOUNCEMENT: ", "Car Wipe In 20 Seconds!"}
					})
			Citizen.Wait(20000)
			TriggerClientEvent("wld:carwipe", -1) 
			TriggerClientEvent('chat:addMessage', -1, {
