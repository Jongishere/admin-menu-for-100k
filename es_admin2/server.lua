...
TriggerEvent('es:addCommand', 'report', function(source, args, user)
	TriggerClientEvent('chat:addMessage', source, {
template = '<div style="padding: 0.6vw; margin: 0.6vw; background-color: rgba(255, 0, 0, 0.6); border-radius: 3px;"> Report Created! {1}</div>',         args = {"^1REPORT INCOMING", "[" .. GetPlayerName(source) .. " - ID: " .. source .. "^0] " .. table.concat(args, " ")}     })
...
-- Noclip
TriggerEvent('es:addGroupCommand', 'fly', "superadmin", function(source, args, user)
...
TriggerEvent('es:addGroupCommand', 'announce', "admin", function(source, args, user)
    TriggerClientEvent('chat:addMessage', -1, {
        template = '<div style="padding: 0.41vw; margin: -0.1vw; background-color: rgba(255, 0, 0, 0.6); border-radius: 3px;"><i class="fas fa-bell"></i> {0} {1}</div>',
        args = { "Cloud RP Announcement :", table.concat(args, " ") }
    })
end, function(source, args, user)
...
Citizen.CreateThread(
    function()
        while true do
            Citizen.Wait(500)
            _players = {}
            if GetNumPlayerIndices() ~= 0 and GetNumPlayerIndices ~= nil then
                local srcs = {};
                ...
                for _, i in pairs(srcs) do
                    local src = i
                    if src ~= nil then
                        local ped = GetPlayerPed(src)
                        _players[tonumber(src)] = {
                            ["src"] = src,
